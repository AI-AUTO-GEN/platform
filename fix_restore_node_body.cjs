...
async function run() {
  ...
  if (restoreNode) {
    restoreNode.parameters.jsCode = `return [{ json: { body: $node['Restore Payload & Auth'].json } }];`;
  }
  ...
  console.log('Successfully fixed Restore Billing Payload node with body wrapper!');
}
